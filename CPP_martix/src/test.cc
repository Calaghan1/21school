...
TEST(GetterAndSetter, set_rows_0) {
  ...
  S21Matrix tmp(4, 5);
  S21Matrix tmp2(4, 4);
  EXPECT_THROW(basic.set_rows(0), std::out_of_range);
  EXPECT_THROW(basic.set_columns(0), std::out_of_range);
  basic.set_rows(4);
...
//   S21Matrix basic;
//   EXPECT_EQ(basic.get_rows(), 0);
//   EXPECT_EQ(basic.get_columns(), 0);
// }
...
TEST(GetterAndSetter, True) {
  ...
  ASSERT_EQ(matrix_a.get_columns(), 3);
}
...
TEST(Constructors, copy_constructor) {
  ...
  EXPECT_EQ(result.get_columns(), 3);
